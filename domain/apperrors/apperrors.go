...
const (
	...
	NotAcceptable        Type = "NOT_ACCEPTABLE"         // for http 406
)
...
// NewNotAcceptable to create an error for 406
func NewNotAcceptable(reason string) *Error {
	return &Error{
		Type:    NotAcceptable,
		Message: reason,
	}
...
func NewInternalWithTrace(trace string) *Error {
	return &Error{
		Type:    Internal,
		Message: "Internal server error. trace -> " + trace,
	}
}
...
// NewNotFound to create an error for 404
func NewNotFoundReason(reason string) *Error {
	return &Error{
		Type:    NotFound,
		Message: reason,
	}
